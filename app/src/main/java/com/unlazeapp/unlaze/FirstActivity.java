                    ...
                    for (int i = 0; i < GlobalVars.getInstance().activityIcons.length; i++) {
                        ...
                        activityButton[i].setOnClickListener(new View.OnClickListener() {
                            public void onClick(View v) {
                                ...
                                // fade out all
                                for (int k = 0; k < 10; k++) {
                                    activityButton[k].getBackground().setAlpha(66);
                                }
    ...
    public class ImageAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            if (pagerViews[position].getDrawable() == null) {
                try {
                    imageLoader.displayImage(GlobalVars.getInstance().userDetail.getJSONArray("face").getJSONObject(position).getString("source"), pagerViews[position]);
                } catch (JSONException e) {
                    e.printStackTrace();
                }
